...
export class PopulationRepository {
  ...
  async findBest(): Promise<PopulationEntity[] | null> {
    const allFactionFights = this._repository
      ...
      .andWhere('nc > 12')
      .andWhere('tr > 12')
      .andWhere('ABS(1 - (vs / nc)) < 0.2')
      .andWhere('ABS(1 - (vs / tr)) < 0.2')
      .andWhere('ABS(1 - (nc / tr)) < 0.2')
      .getMany()
  ...
  _findTwoFactionFight(faction1: string, faction2: string): Promise<PopulationEntity[] | null> {
    return this._repository.createQueryBuilder().select().where(`${faction1} > 12`).andWhere(`${faction2} > 12`).andWhere(`ABS(1 - (${faction1} / ${faction2})) < 0.2`).getMany()
  }
}
